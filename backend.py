...
class Classroom:
    ...
    def read_existing_data(self, active_file, hold_file):
        ...
        active_df = active_df.rename(columns={'First Name': 'Name', 'Dob': 'DoB', 'Room': 'Room', 'Time Schedule': 'Schedule', 'Tags': 'Program Type'})
        active_df['Program Type'] = active_df['Program Type'].str.contains('FlexEd').map({True: 'Flexible', False: 'Fixed'})
        ...
        hold_df = hold_df.rename(columns={'First Name': 'Name', 'Dob': 'DoB', 'Room': 'Room', 'Time Schedule': 'Schedule', 'Tags': 'Program Type', 'Admission Date': 'Admission Date'})
        ...
        for _, row in active_df.iterrows():
            if pd.notna(row['DoB']):
                student = Student(row['Name'], datetime.strptime(str(row['DoB']), '%Y-%m-%d %H:%M:%S'), level_dict[row['Room']], row['Schedule'], row['Program Type'])
        ...
        for _, row in hold_df.iterrows():
            if pd.notna(row['DoB']) and pd.notna(row['Admission Date']):
                student = Student(row['Name'], datetime.strptime(str(row['DoB']), '%Y-%m-%d %H:%M:%S'), level_dict[row['Room']], row['Schedule'], row['Program Type'], datetime.strptime(str(row['Admission Date']), '%Y-%m-%d %H:%M:%S'))
                student.promotion_date = self.calculate_promotion_date(student)
        ...
        for student in self.students:
            if student.existing_student and student.schedule is not None:
                for day in student.schedule:
                    daily_strength[day.strip()][student.level] += 1
        return pd.DataFrame(daily_strength)
    ...
    def apply_for_admission(self, applicant, joining_date=None):
        ...
        flexible_students = [student for student in self.students if student.existing_student and student.level == level and student.program_type == "Flexible"]
    ...
    def update_waiting_list(self, level):
        if self.level_promotedQueues[level]:
            self.level_promotedQueues[level] = deque(sorted(self.level_promotedQueues[level], key=lambda x: x.start_date))
    ...
    def admit_students_from_waiting(self, level, joining_date):
        while all(self.calculate_daily_strength().loc[level] != 0):
            if self.level_promotedQueues[level]:
    ...
    def can_join_level(self, schedule, level):
        ...
        for day in schedule:
            students_in_level = sum(1 for student in self.students if student.level == level and day.lower() in [d.lower() for d in student.schedule])
    ...
    def calculate_next_possible_dates(self, level, joining_date):
        nextPromotedDates = []
        if level > 2:
            nextPromotedDates.extend(student.promotion_date for student in self.students if student.level == level - 2 and student.promotion_date >= joining_date)
            nextPromotedDates.extend(student.promotion_date for student in self.level_queues[level - 2] if student.promotion_date >= joining_date)
        if level > 1:
            nextPromotedDates.extend(student.promotion_date for student in self.students if student.level == level - 1 and student.promotion_date >= joining_date)
            nextPromotedDates.extend(student.promotion_date for student in self.level_queues[level - 1] if student.promotion_date >= joining_date)
        nextPromotedDates.extend(student.promotion_date for student in self.students if student.level == level and student.promotion_date >= joining_date)
        nextPromotedDates.extend(student.promotion_date for student in self.level_queues[level] if student.promotion_date >= joining_date)
    ...
    def date_strength(self, level):
        ...
        while current_date <= end_date:
            ...
            count = sum(1 for student in self.students if student.existing_student and student.level == level and day_of_week in student.schedule)
...
def process_inputs(name, dob, child_class, schedule_days, program_type, joining_date, active_file, hold_file, fte_file):
    ...
    metrics = {
        ...
        'Schedule Available': df.to_string()
    }
...
if __name__ == "__main__":
    ...
    program_type = "Fixed"
    joining_date = datetime(2024, 6, 1)
    active_file = "path/to/active_list.xlsx"
    hold_file = "path/to/hold_list.xlsx"
    fte_file = "path/to/fte_list.xlsx"

    metrics = process_inputs(name, dob, child_class, schedule_days, program_type, joining_date, active_file, hold_file, fte_file)
    print(metrics)